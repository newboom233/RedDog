...
const LCUMonitor = () => {
    ...
    <div className="connection-status">
      ...
      <div className="status-grid">
        <div className="status-item">
          ...
          <span className={`value ${connectionStatus.isConnected ? 'connected' : 'disconnected'}`}>
        ...
        <div className="status-item">
          ...
          <span className={`value ${connectionStatus.hasPassword ? 'success' : 'error'}`}>
          ...
          <>
            ...
            <button 
              onClick={() => {
                ...
                alert(`LCU服务数据: ${JSON.stringify(lcuService.enemyInfo, null, 2)}\n\nReact状态: ${JSON.stringify(enemyInfo, null, 2)}\n\n强制更新: ${forceUpdate}\n\n监听器数量: ${lcuService.listeners.length}\n\n连接状态: ${JSON.stringify(lcuService.connectionStatus, null, 2)}\n\n轮询状态: ${!!lcuService.pollingInterval}\n\n最后游戏阶段: ${lcuService.lastGamePhase}`);
              }} 
              className="debug-btn"
            ...
            <button 
              onClick={async () => {
                ...
                try {
                  ...
                  // 测试英雄选择会话
                  const champSelectResponse = await lcuService.makeRequest('/lol-champ-select/v1/session');
                  ...
                  alert(`测试完成！\n\n召唤师信息: ${JSON.stringify(currentSummoner, null, 2)}\n\n游戏状态: ${JSON.stringify(gameStatus, null, 2)}`);
                } catch (err) {
              ...
              className="test-btn"
              style={{ background: '#9c27b0', color: 'white', border: 'none', padding: '8px 12px', borderRadius: '4px', cursor: 'pointer' }}
            >
              🧪 测试技能
            </button>
            <button 
              ...
              style={{ background: '#ff9800', color: 'white', border: 'none', padding: '8px 12px', borderRadius: '4px', cursor: 'pointer' }}
            >
              🔍 诊断问题
            </button>
            <button 
              onClick={async () => {
                ...
                try {
                  ...
                  const gameSessionResponse = await lcuService.makeRequest('/lol-gameflow/v1/session');
              ...
              style={{ background: '#673ab7', color: 'white', border: 'none', padding: '8px 12px', borderRadius: '4px', cursor: 'pointer' }}
            >
              🧪 测试数据
            </button>
            <button 
              ...
              style={{ background: '#e91e63', color: 'white', border: 'none', padding: '8px 12px', borderRadius: '4px', cursor: 'pointer' }}
            >
              🎨 测试UI
            </button>
          </>
      ...
      <div className="summoner-info">
        ...
        <div className="info-grid">
          <div className="info-item">
            ...
            <span className="value">{summonerInfo.gameName || summonerInfo.displayName || summonerInfo.name || '未知'}</span>
          </div>
          <div className="info-item">
            <span className="label">等级:</span>
            <span className="value">{summonerInfo.summonerLevel || '未知'}</span>
          </div>
          <div className="info-item">
          ...
          {summonerInfo.puuid && (
            <div className="info-item">
              ...
              <span className="value">{summonerInfo.puuid.substring(0, 8)}...</span>
            </div>
          )}
        ...
        <div className="info-grid">
          ...
          {gameStatus.gameData && (
            <>
              ...
              <div className="info-item">
                ...
                <span className="value">{gameStatus.gameData.queue?.description || '未知'}</span>
              </div>
            </>
          )}
          {gameStatus.playerChampionSelections && (
            <div className="info-item">
              ...
              <span className="value">{Object.keys(gameStatus.playerChampionSelections).length}个选择</span>
            </div>
          )}
        </div>
        ...
        <div className="enemy-info">
          ...
          <div className="no-data">
            ...
            <button 
              ...
              style={{ background: '#ff5722', color: 'white', border: 'none', padding: '8px 12px', borderRadius: '4px', cursor: 'pointer', marginLeft: '10px' }}
            >
              🔄 强制渲染
            </button>
          </div>
    ...
    return (
      <div className="enemy-info">
        ...
        <div className="info-type">
          <span className="type-badge">{statusText[enemyInfo.type] || enemyInfo.type}</span>
          <span className="timestamp">{new Date(enemyInfo.timestamp).toLocaleTimeString()}</span>
        </div>
              ...
              {enemyInfo.theirTeam.map((player, index) => (
                <div key={index} className="player-card enemy">
                  <div className="player-header">
                    <div className="player-name">{player.summonerName || player.playerName || '未知玩家'}</div>
                    <div className="player-position">{player.assignedPosition || '未知位置'}</div>
                  </div>
                  <div className="champion-selection">
                    {player.championId ? (
                      <div className="champion-info">
                        <div className="champion-name">{player.championName || `英雄ID: ${player.championId}`}</div>
                        <div className="champion-id">ID: {player.championId}</div>
                      </div>
                    ) : (
                      ...
                      <div className="spell-item">
                        <div className="spell-icon">
                          {player.summonerSpells?.spell1 === '闪现' ? '⚡' :
                           ...
                           player.summonerSpells?.spell1 === '屏障' ? '🛡️' :
                           player.summonerSpells?.spell1 === '净化' ? '✨' :
                           player.summonerSpells?.spell1 === '惩戒' ? '⚔️' :
                           ...
                           player.summonerSpells?.spell1 === '洞察' ? '👁️' :
                           '❓'}
                        </div>
                        <div className="spell-details">
                          <span className="spell-name">{player.summonerSpells?.spell1 || '未选择'}</span>
                          <span className="spell-id">{player.spell1Id ? `ID: ${player.spell1Id}` : ''}</span>
                        </div>
                      </div>
                      <div className="spell-item">
                        <div className="spell-icon">
                          {player.summonerSpells?.spell2 === '闪现' ? '⚡' :
                           ...
                           player.summonerSpells?.spell2 === '屏障' ? '🛡️' :
                           player.summonerSpells?.spell2 === '净化' ? '✨' :
                           player.summonerSpells?.spell2 === '惩戒' ? '⚔️' :
                           ...
                           player.summonerSpells?.spell2 === '洞察' ? '👁️' :
                           '❓'}
                        </div>
                        <div className="spell-details">
                          <span className="spell-name">{player.summonerSpells?.spell2 || '未选择'}</span>
                          <span className="spell-id">{player.spell2Id ? `ID: ${player.spell2Id}` : ''}</span>
                        </div>
                      </div>
                    </div>
                    {/* 调试信息 */}
                    <div className="spell-debug" style={{ fontSize: '10px', color: '#666', marginTop: '5px' }}>
                      <div>技能1: {player.spell1Id || 'null'} → {player.summonerSpells?.spell1 || 'null'}</div>
                      <div>技能2: {player.spell2Id || 'null'} → {player.summonerSpells?.spell2 || 'null'}</div>
                    </div>
                  </div>
                  ...
                  <div className="player-details">
                    <div className="detail-item">
                      ...
                      <span className="value">{player.summonerId || '未知'}</span>
                    </div>
                    {player.cellId && (
                      <div className="detail-item">
  ...
  return (
    <div className="lcu-monitor">
      ...
      {/* 原始数据显示区域 */}
      <div className="raw-data-display" style={{ 
        background: '#f5f5f5', 
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px' }}>
          {/* LCU服务数据 */}
          <div>
            <h4 style={{ margin: '0 0 5px 0', color: '#e74c3c' }}>LCU服务数据:</h4>
            <div style={{ 
              background: '#fff', 
              padding: '10px', 
              ...
              <div><strong>连接状态:</strong> {lcuService.isConnected ? '✅ 已连接' : '❌ 未连接'}</div>
              <div><strong>端口:</strong> {lcuService.port || 'null'}</div>
              <div><strong>enemyInfo:</strong> {lcuService.enemyInfo ? '✅ 有数据' : '❌ 无数据'}</div>
              <div><strong>监听器数量:</strong> {lcuService.listeners.length}</div>
              <div><strong>最后游戏阶段:</strong> {lcuService.lastGamePhase || 'null'}</div>
              <hr style={{ margin: '10px 0' }} />
              <div><strong>enemyInfo详情:</strong></div>
              <pre style={{ 
                margin: '5px 0', 
                whiteSpace: 'pre-wrap', 
                wordBreak: 'break-all',
          ...
          {/* React状态数据 */}
          <div>
            <h4 style={{ margin: '0 0 5px 0', color: '#3498db' }}>React状态数据:</h4>
            <div style={{ 
              background: '#fff', 
              padding: '10px', 
              border: '1px solid #ddd', 
              ...
              <div><strong>enemyInfo:</strong> {enemyInfo ? '✅ 有数据' : '❌ 无数据'}</div>
              <div><strong>gameStatus:</strong> {gameStatus ? gameStatus.phase : 'null'}</div>
              <div><strong>summonerInfo:</strong> {summonerInfo ? '✅ 有数据' : '❌ 无数据'}</div>
              <div><strong>强制更新计数:</strong> {forceUpdate}</div>
              <div><strong>连接状态:</strong> {connectionStatus.isConnected ? '✅ 已连接' : '❌ 未连接'}</div>
              <hr style={{ margin: '10px 0' }} />
              <div><strong>enemyInfo详情:</strong></div>
              <pre style={{ 
                margin: '5px 0', 
        ...
        <div style={{ 
          ...
          <h4 style={{ margin: '0 0 10px 0', color: '#2c3e50' }}>📊 状态摘要:</h4>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '10px' }}>
            <div><strong>LCU连接:</strong> <span style={{ color: lcuService.isConnected ? '#27ae60' : '#e74c3c' }}>
              {lcuService.isConnected ? '✅ 已连接' : '❌ 未连接'}
            </span></div>
            <div><strong>LCU数据:</strong> <span style={{ color: lcuService.enemyInfo ? '#27ae60' : '#e74c3c' }}>
              {lcuService.enemyInfo ? '✅ 有数据' : '❌ 无数据'}
            </span></div>
            <div><strong>React数据:</strong> <span style={{ color: enemyInfo ? '#27ae60' : '#e74c3c' }}>
              {enemyInfo ? '✅ 有数据' : '❌ 无数据'}
            </span></div>
            <div><strong>游戏阶段:</strong> <span style={{ color: gameStatus ? '#27ae60' : '#e74c3c' }}>
              {gameStatus ? gameStatus.phase : '❌ 未知'}
            </span></div>
            <div><strong>数据同步:</strong> <span style={{ color: (lcuService.enemyInfo && enemyInfo) ? '#27ae60' : '#e74c3c' }}>
              {(lcuService.enemyInfo && enemyInfo) ? '✅ 已同步' : '❌ 未同步'}
            </span></div>
            <div><strong>更新次数:</strong> <span style={{ color: '#3498db' }}>
              {forceUpdate}
            </span></div>
          </div>
        </div>
      ...
      <div className="debug-status" style={{ 
        ...
        <strong>调试状态:</strong>
        <div>enemyInfo: {enemyInfo ? `有数据 (${enemyInfo.type})` : 'null'}</div>
        <div>gameStatus: {gameStatus ? gameStatus.phase : 'null'}</div>
        <div>connectionStatus: {connectionStatus.isConnected ? '已连接' : '未连接'}</div>
